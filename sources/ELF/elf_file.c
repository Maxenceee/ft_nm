...
new_elf_file(t_binary_reader *reader)
{
	...
	if (elf_file->e_phoff > reader->size
		...
		|| (elf_file->e_ehsize != 0 && elf_file->e_ehsize != ELF_HEADER_SIZE)
		|| (elf_file->e_phentsize != 0 && elf_file->e_phentsize != ELF_PROGRAM_HEADER_SIZE)
		|| (elf_file->e_shentsize != 0 && elf_file->e_shentsize != ELF_SECTION_HEADER_SIZE)
		|| elf_file->e_shstrndx > elf_file->e_shnum
		|| elf_file->e_shnum > reader->size / ELF_PROGRAM_HEADER_SIZE
		|| elf_file->e_phnum > reader->size / ELF_SECTION_HEADER_SIZE
	)
	{
		delete_elf_file(elf_file);
		return (ft_dverbose(STDERR_FILENO, "Could not read file because of incoeherent values\n"), NULL);
...
void
delete_elf_file(t_elf_file *elf_file)
{
	if (elf_file == NULL)
		return ;
